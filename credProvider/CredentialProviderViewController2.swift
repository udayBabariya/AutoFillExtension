//
//  CredentialProviderViewController2.swift
//  credProvider
//
...
class CredentialProviderViewController2: ASCredentialProviderViewController {
    ...
    override func provideCredentialWithoutUserInteraction(for credentialIdentity: ASPasswordCredentialIdentity) {
        ...
        print("in provider: \(credentialIdentity.serviceIdentifier.identifier)")
...
//        let a = credentialIdentity.serviceIdentifier.identifier
//
        if let allCreds = PlistManager.load(), allCreds.count > 0{
            let filteredCreds = allCreds.filter {$0.domain == credentialIdentity.serviceIdentifier.identifier}
...
extension CredentialProviderViewController2: addCredVCDelegate{
    func credAdded() {
        fetchPasswords_ReloadTV()
    }
}
